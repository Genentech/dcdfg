import numpy as np
from causaldag import igsp, unknown_target_igsp
from conditional_independence import (MemoizedCI_Tester,
                                      MemoizedInvarianceTester,
                                      gauss_invariance_suffstat,
                                      gauss_invariance_test, hsic,
                                      hsic_invariance_test, hsic_test, kci,
                                      kci_invariance_test, kci_test,
...
def prepare_igsp(
    ...
    if ci_test == "gaussian":
        ...
        invariance_tester = MemoizedInvarianceTester(
            hsic_invariance_test, invariance_suffstat, alpha=alpha_inv
        )
    elif ci_test == "kci":
        contexts = {i: s for i, s in enumerate(iv_samples_list)}
        invariance_suffstat = {"obs_samples": obs_samples}
        ...
        invariance_tester = MemoizedInvarianceTester(
            kci_invariance_test, invariance_suffstat, alpha=alpha_inv
        )
    else:
        raise ValueError(
...
def run_igsp(data, targets, regimes, alpha=1e-3, alpha_inv=1e-3, ci_test="gaussian"):
    ...
    nodes, obs_samples, iv_samples_list, targets_list = format_to_igsp(
        data, targets, regimes
    )
    ci_tester, invariance_tester = prepare_igsp(
        obs_samples, iv_samples_list, targets_list, alpha, alpha_inv, ci_test
    )
